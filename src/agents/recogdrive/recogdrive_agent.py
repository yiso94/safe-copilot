...
class ReCogDriveAgent(AbstractAgent):
    def __init__(
        ...
        if not self.cache_hidden_state and not self.cache_mode:
            ...
            self._validate_backbone_configuration()
            self.backbone = self._build_backbone(device)
    ...
    def forward(
        ...
        *,
        ...
        if self.cache_hidden_state:
            ...
            last_hidden_state = self._compute_last_hidden_state_from_backbone(
                features=features,
    ...
    def _build_backbone(self, device: str) -> RecogDriveBackbone:
        return RecogDriveBackbone(
            model_type=self.vlm_type,
            checkpoint_path=self.vlm_path,
            device=device,
        )
    ...
    def _compute_last_hidden_state_from_backbone(
        self,
        *,
        features: Dict[str, torch.Tensor],
        history_trajectory: torch.Tensor,
        ...
        questions = self._build_questions(history_trajectory, high_command_one_hot)
