...
class QWenInfer:
    ...
    def get_model(self):
        ...
        gemm_allreduce_plugin = config["build_config"]["plugin_config"]["gemm_allreduce_plugin"]
        ...
        max_prompt_embedding_table_size = config["build_config"].get("max_prompt_embedding_table_size", 0)
        ...
        if config["pretrained_config"].get("multi_query_mode", False):
            tensorrt_llm.logger.warning("`multi_query_mode` config is deprecated. Please rebuild the engine.")
            num_kv_heads = 1
    ...
    def ptuning_setup(self, prompt_table, dtype, hidden_size, tasks, input_ids):
        ...
        if tasks is not None:
            ...
            assert tasks.shape[0] == input_ids.shape[0], "Number of supplied tasks must match input batch size"
        else:
    ...
    def generate_for_qwenvl(
        ...
        input_lengths = torch.tensor([input_ids.size(1)], device="cuda", dtype=torch.int32)
        max_input_length = torch.max(input_lengths).item()
        max_new_tokens = min(max_new_tokens, self.global_max_input_len - max_input_length)
    ...
    def qwen_infer(
        ...
        output_ids, Qwen_time = self.generate_for_qwenvl(
            input_ids, max_new_tokens, prompt_table, tasks, task_vocab_size, num_beams
        )
...
def vit_process(image_path, vit_engine_path, stream):
    ...
    visual_output_info = session_vit.infer_shapes([TensorInfo("input", trt.DataType.FLOAT, images.shape)])
    visual_outputs = {
