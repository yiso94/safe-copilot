...
CONFIG_PATH = "../../configs/navsim/pdm_scoring"
...
def run_pdm_score(args: list[dict[str, list[str] | DictConfig]]) -> list[dict[str, Any]]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        if dist.get_rank() == 0:
            logger.info(
                f"Rank {dist.get_rank()} processing scenario {idx + 1} / {len(tokens_to_evaluate)} in thread_id={thread_id}, node_id={node_id}"
            )
